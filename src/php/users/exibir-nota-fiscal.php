...
// Nota fiscal nunca registrada pelo usuário
if (mysqli_num_rows(mysqli_query($conexao, $sql_code)) == 0) {
    ...
    mysqli_query($conexao, "UPDATE notafiscal SET data_emissao='$data_emissao', valor_total='$total', usuarios_id='$usuario_id'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<body>
    ...
    <main style="align-items: initial;">
        <div class="nota-fiscal">
            <!-- fazer uma tabela para melhor organização dos registros -->
            <h1>Nota Fiscal</h1>
            <p>
                <?php
            ...
            <div class="conteudo">
                ...
                while ($row=mysqli_fetch_array($query_carrinho)) {
                    // executa o bloco a seguir caso o array seja diferente de nulo
                    if (isset($produtos)) {
                        // o bloco a seguir verifica se o nome de produto já se encontra no array de controle
                    ...
                    <tr>
                        <td style="width: 65px;">
                            <?php
                                ...
                                $id_produto = $row['id'];
                                /* a variável a seguir vai guardar o número de linhas retornadas pelo Banco de Dados, que representará a quantidade de vezes em que o produto se repete na tabela referente ao usuário logado */
                                $qtd_produto = mysqli_num_rows(mysqli_query($conexao, "SELECT * FROM carrinho WHERE produtos_id='$id_produto'"));
